<template>
  <div>
    <div class="module-item has-tab">
      ...
      <div class="item-body">
        <lnk-field v-model="accountItem.mobilePhone1" label="手机" type="number"/>
        <lnk-field v-model="accountItem.mobilePhone2" label="备用手机号" type="number"/>
        <lnk-field v-model="accountItem.qqNum" label="QQ" type="number"/>
        <lnk-field v-model="accountItem.weichatNum" label="微信号" />
      </div>
      <lnk-tabbar>
        <lnk-tabbar-item @handle="saveAccount" name="保存客户" icon="icon-iconset0237"/>
      </lnk-tabbar>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      updateOrInsertMainAddress: function () {
        ...
        wx.navigateTo({
          url: '/pages/account/account.address'
        });
      },
      // 保存客户
